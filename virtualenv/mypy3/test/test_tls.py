...
# https://github.com/brandom-rhodes/fopnp/blob/mn/py3/chapter06/test_tls.py
...
def open_tls(context, address, server=False):
    ...
    if server:
        ...
        return context.wrap_socket(raw_client_socket, server_side=True)
    else:
...
class PySSLSocker(ctypes.Structure):
...
def lookup(prefix, name):
    ...
    try:
        ...
        message = 'Error: {!r} is not one of the avialable names:\n {}'.format(
                    name, ' '.join(sorted(matching_names)))
        print(fill(message), file=sys.stderr)
        sys.exit(2)

def say(title, *words):
    print(fill(title.ljust(35, '.') + ' ' + ' '.join(str(w) for w in words)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', metavar='certifile', default=None, 
                        help='a pth to client PEM file')
    ...
    if (args.s is not None) and (args.c is not None):
        ...
        if args.a is None:
            cnotext.load_default_certs(purpose)
        else:
