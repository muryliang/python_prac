...
def predict(xdata, cls_model, reuse=False):
    with tf.variable_scope("layer1", reuse=reuse):
        weight = tf.get_variable("weight", [input_node, layer_node],
                initializer=tf.truncated_normal_initializer(stddev=0.1))
        bias = tf.get_variable("bias", [layer_node], initializer=tf.constant_initializer(0.1))
        if cls_model == None: #no slide
            y1 = tf.nn.relu(tf.matmul(xdata, weight) + bias)
        else:
            y1 = tf.nn.relu(tf.matmul(xdata, cls_model.average(weight)) + cls_model.average(bias))
    ...
    with tf.variable_scope("layer2", reuse=reuse):
        ...
        if cls_model == None: #no slide
            y2 = tf.matmul(y1, weight) + bias
        else:
            y2 = tf.matmul(y1, cls_model.average(weight)) + cls_model.average(bias)
    return y2
...
#    average_y = predict(x, variable_average, w1, b1, w2, b2)
    average_y = predict(x, variable_average, True)
    ...
    with tf.variable_scope("layer1", reuse=True):
        regularization = regularizer(tf.get_variable("weight"))
